...
$(outdir)/org/idpass/tools/*.class: src/main/java/org/idpass/tools/*.java
	javac -source $(javasource) -target $(javatarget) -d $(outdir) -cp $(gp211) src/main/java/org/idpass/tools/*.java

$(outdir)/org/idpass/auth/*.class: src/main/java/org/idpass/auth/*.java
	javac -source $(javasource) -target $(javatarget) -cp $(jars) -d $(outdir) src/main/java/org/idpass/auth/*.java

$(outdir)/org/idpass/sam/*.class: src/main/java/org/idpass/sam/*.java
	javac -source $(javasource) -target $(javatarget) -cp $(jars) -d $(outdir) src/main/java/org/idpass/sam/*.java
